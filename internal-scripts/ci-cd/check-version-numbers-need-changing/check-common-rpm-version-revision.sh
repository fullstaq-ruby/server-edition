#!/bin/bash
# Checks whether the fullstaq-ruby-common RPM package
...
# RPM package's contents or metadata.

REVIEW_GLOB="container-entrypoints/build-common-rpm"
# shellcheck disable=SC2207,SC2012
REVIEW_FILES=($(ls container-entrypoints/build-common-rpm | sort))
...
# A relevant script changed. So `common.rpm.(version|package_revision)`
# may need bumping.
...
CURRENT_COMMON_PACKAGE_VERSION=$(ruby -ryaml -e 'puts YAML.load_file("config.yml")["common"]["rpm"]["version"]')
CURRENT_COMMON_PACKAGE_REVISION=$(ruby -ryaml -e 'puts YAML.load_file("config.yml")["common"]["rpm"]["package_revision"]')
LATEST_RELEASE_COMMON_PACKAGE_VERSION=$(git archive "$LATEST_RELEASE_TAG" config.yml | tar -xO | ruby -ryaml -e 'puts YAML.load(STDIN)["common"]["rpm"]["version"]')
LATEST_RELEASE_COMMON_PACKAGE_REVISION=$(git archive "$LATEST_RELEASE_TAG" config.yml | tar -xO | ruby -ryaml -e 'puts YAML.load(STDIN)["common"]["rpm"]["package_revision"]')

if [[ "$CURRENT_COMMON_PACKAGE_VERSION" != "$LATEST_RELEASE_COMMON_PACKAGE_VERSION" ]]; then
    echo " * The fullstaq-ruby-common RPM package version has been changed compared to $LATEST_RELEASE_TAG."
    echo
    ...
    if [[ "$CURRENT_COMMON_PACKAGE_REVISION" = 0 ]]; then
        echo " * The fullstaq-ruby-common RPM package revision is properly set to 0."
        exit
    else
        echo " * The fullstaq-ruby-common RPM package revision is not set to 0."
        echo
        echo "${BOLD}${YELLOW}*** ACTION REQUIRED ***${RESET}"
        echo "Please edit config.yml and change ${BOLD}common.rpm.package_revision${RESET} to 0."
        exit 1
    fi
fi
if [[ "$CURRENT_COMMON_PACKAGE_REVISION" -gt "$LATEST_RELEASE_COMMON_PACKAGE_REVISION" ]]; then
    echo " * The fullstaq-ruby-common RPM package revision has been bumped compared"
...
echo " * Neither the fullstaq-ruby-common RPM package version nor its revision has"
...
APPROVAL_DATA=$(
    ...
    echo "component=common-rpm-version-revision" &&
...
if grep -q "^${APPROVAL_CHECKSUM}$" approvals.txt; then
    ...
    echo
    echo "Check whether the code would ${BOLD}change the fullstaq-ruby-common RPM"
    ...
    echo "   config.yml and bump either common.rpm.version or"
    echo "   common.rpm.package_revision."
    echo
    echo "  ${BOLD}-- OR --${RESET}"
    echo
    ...
    echo "     git commit -a -m 'Approve fullstaq-ruby-server-edition common-rpm-version-revision $HEAD_SHA_SHORT' &&"
    echo "     git push"
    exit 1
fi
